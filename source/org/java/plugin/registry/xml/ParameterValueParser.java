/*****************************************************************************
 * Java Plug-in Framework (JPF) Copyright (C) 2006-2007 Dmitry Olshansky This
...
{
	private static ExtensionPoint getExtensionPoint(final PluginRegistry registry,
	...
	ParameterValueParser(final PluginRegistry registry, final ParameterDefinition definition,
		final String rawValue)
	{
		...
		switch( definition.getType() )
		{
			...
			case NUMBER:
				try
				{
					value = NumberFormat.getInstance(Locale.ENGLISH).parse(val);
				}
				...
				break;
			case DATE: {
				DateFormat fmt = new SimpleDateFormat("yyyy-MM-dd", Locale.ENGLISH); //$NON-NLS-1$
				try
				{
					value = fmt.parse(val);
				}
			...
			case TIME: {
				DateFormat fmt = new SimpleDateFormat("HH:mm:ss", Locale.ENGLISH); //$NON-NLS-1$
				try
				{
					value = fmt.parse(val);
				}
			...
			case DATE_TIME: {
				DateFormat fmt = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.ENGLISH); //$NON-NLS-1$
				try
				{
					value = fmt.parse(val);
				}
			...
			case EXTENSION_POINT_ID:
				...
				{
					ExtensionPoint customExtPoint = getExtensionPoint(registry,
						definition.getCustomData());
					if( customExtPoint == null )
					{
						parsingMessage = "unknown extension point UID " //$NON-NLS-1$
							+ definition.getCustomData() + " provided as custom data"; //$NON-NLS-1$
						isParsingSucceeds = false;
						return;
					}
					if( !((ExtensionPoint) value).isSuccessorOf(customExtPoint) )
					{
			...
			case EXTENSION_ID:
				...
				for( Extension ext : registry.getPluginDescriptor(registry.extractPluginId(val))
					.getExtensions() )
				{
				...
				if( definition.getCustomData() != null )
				{
					...
					ExtensionPoint extPoint = getExtensionPoint(registry, extPointUid);
					if( extPoint == null )
					{
					...
					if( !extPoint.equals(customExtPoint) && !extPoint.isSuccessorOf(customExtPoint) )
					{
				...
				break;
			case FIXED:
				for( StringTokenizer st = new StringTokenizer(definition.getCustomData(),
					"|", false); //$NON-NLS-1$
				st.hasMoreTokens(); )
				{
