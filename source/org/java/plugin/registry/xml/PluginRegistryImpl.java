 ...
 * <dl>
 * <dt>isValidating</dt>
 * <dd>Regulates is registry should use validating parser when loading plug-in
 * manifests. The default parameter value is <code>true</code>.</dd>
 * <dt>stopOnError</dt>
 * <dd>Regulates is registry should stop and throw RuntimeException if an error
 * occurred while {@link PluginRegistry#register(URL[]) registering} or
 * {@link PluginRegistry#unregister(String[]) un-registering} plug-ins. If this
 * is <code>false</code>, the registration errors will be stored in the internal
 * report that is available with
 * {@link PluginRegistry#checkIntegrity(PathResolver)} method. The default
 * parameter value is <code>false</code>.</dd>
 * </dl>
 * 
...
public final class PluginRegistryImpl implements PluginRegistry
{
	...
	private final List<RegistryChangeListener> listeners = Collections
		.synchronizedList(new LinkedList<RegistryChangeListener>());
	...
	{
		registrationReport.add(new ReportItemImpl(IntegrityCheckReport.Severity.INFO, null,
			IntegrityCheckReport.Error.NO_ERROR, "registryStart", null)); //$NON-NLS-1$
	}

	/**
	 * @see org.java.plugin.registry.PluginRegistry#configure(ExtendedProperties)
	 */
	...
	/**
	 * @see org.java.plugin.registry.PluginRegistry#readManifestInfo(java.net.URL)
	 */
	public ManifestInfo readManifestInfo(final URL url) throws ManifestProcessingException
	{
		...
		{
			throw new ManifestProcessingException(PACKAGE_NAME, "manifestParsingError", url, e); //$NON-NLS-1$
		}
	}
	 ...
	 * <li>Handle versions correctly - register new descriptor as most recent
	 * version or as an old version.</li>
	 * <li>If other versions of the same plug-in already registered, take their
	 * fragments and register them with this version.</li>
	 * </ol>
	 * </li>
	 * <li>Process new plug-in fragments next:
	 * <ol>
	 * <li>Instantiate new PluginFragmentImpl object.</li>
	 * <li>Check if older version of the same fragment already registered. If
	 * yes, un-register it and move to old plug-in fragments collection.</li>
	 * <li>Register new fragment with all matches plug-in descriptors (if this
	 * fragment is of most recent version).</li>
	 * </ol>
	 * </li>
	...
	public Map<String, Identity> register(final URL[] manifests) throws ManifestProcessingException
	{
		...
		for( URL url : manifests )
		{
			...
			catch( Exception e )
			{
				...
				registrationReport.add(new ReportItemImpl(IntegrityCheckReport.Severity.ERROR,
					null, IntegrityCheckReport.Error.MANIFEST_PROCESSING_FAILED,
		...
		registrationReport.add(new ReportItemImpl(IntegrityCheckReport.Severity.INFO, null,
			IntegrityCheckReport.Error.NO_ERROR, "registeringPluginsStart", null)); //$NON-NLS-1$
		for( ModelPluginManifest model : plugins.values() )
		{
			PluginDescriptor descr = registerPlugin((ModelPluginDescriptor) model,
				registryChangeData);
			if( descr != null )
			{
		...
		registrationReport.add(new ReportItemImpl(IntegrityCheckReport.Severity.INFO, null,
			IntegrityCheckReport.Error.NO_ERROR, "registeringFragmentsStart", null)); //$NON-NLS-1$
		for( ModelPluginManifest entry : fragments.values() )
		{
			PluginFragment fragment = registerFragment((ModelPluginFragment) entry,
				registryChangeData);
			if( fragment != null )
			{
		...
		fragments.clear();
		registrationReport.add(new ReportItemImpl(IntegrityCheckReport.Severity.INFO, null,
			IntegrityCheckReport.Error.NO_ERROR, "registeringPluginsFinish", //$NON-NLS-1$
			Integer.valueOf(registeredPlugins.size())));
		registrationReport.add(new ReportItemImpl(IntegrityCheckReport.Severity.INFO, null,
			IntegrityCheckReport.Error.NO_ERROR, "registeringFragmentsFinish", //$NON-NLS-1$
					...
					for( Extension ext : extensionPoint.getConnectedExtensions() )
					{
						if( !registeredExtensions.containsKey(ext.getUniqueId()) )
						{
							registryChangeData.putAddedExtension(ext.getUniqueId(),
								makeUniqueId(ext.getExtendedPluginId(), ext.getExtendedPointId()));
						}
						else
						{
							...
							if( registryChangeData.modifiedPlugins().contains(
								...
								|| registryChangeData.modifiedPlugins().contains(
	...
	{
		Map<String, Object[]> versions = new HashMap<String, Object[]>(); // <ID,
																			// [Version,
																			// URL]>
		Set<String> toBeRemovedUrls = new HashSet<String>();
		for( Iterator<Map.Entry<String, ModelPluginManifest>> it = plugins.entrySet().iterator(); it
			.hasNext(); )
		{
	...
	private PluginDescriptor registerPlugin(final ModelPluginDescriptor model,
		final RegistryChangeDataImpl registryChangeData) throws ManifestProcessingException
	{
		if( log.isDebugEnabled() )
		{
		...
		try
		{
			...
			registrationReport.add(new ReportItemImpl(IntegrityCheckReport.Severity.INFO, null,
				IntegrityCheckReport.Error.NO_ERROR, "pluginRegistered", result.getUniqueId())); //$NON-NLS-1$
		}
		catch( ManifestProcessingException mpe )
		{
			...
			registrationReport.add(new ReportItemImpl(IntegrityCheckReport.Severity.ERROR, null,
				IntegrityCheckReport.Error.MANIFEST_PROCESSING_FAILED, "pluginRegistrationFailed", //$NON-NLS-1$
		...
		try
		{
			...
			if( !isRegistered )
			{
				log.warn("no matching plug-ins found for fragment " //$NON-NLS-1$
					+ result.getUniqueId());
				registrationReport.add(new ReportItemImpl(IntegrityCheckReport.Severity.WARNING,
					null, IntegrityCheckReport.Error.NO_ERROR,
					"noMatchingPluginFound", result.getUniqueId())); //$NON-NLS-1$
			}
			registrationReport.add(new ReportItemImpl(IntegrityCheckReport.Severity.INFO, null,
				IntegrityCheckReport.Error.NO_ERROR, "fragmentRegistered", result.getUniqueId())); //$NON-NLS-1$
		}
		catch( ManifestProcessingException mpe )
		{
	...
	public Collection<String> unregister(final String[] ids)
	{
		...
		for( String element : ids )
		{
			...
			registrationReport.add(new ReportItemImpl(IntegrityCheckReport.Severity.WARNING, null,
				IntegrityCheckReport.Error.NO_ERROR, "pluginToUngregisterNotFound", element)); //$NON-NLS-1$
		}
		...
		fireEvent(registryChangeData);
		registrationReport.add(new ReportItemImpl(IntegrityCheckReport.Severity.INFO, null,
			IntegrityCheckReport.Error.NO_ERROR, "unregisteringFragmentsStart", null)); //$NON-NLS-1$
		...
		registrationReport.add(new ReportItemImpl(IntegrityCheckReport.Severity.INFO, null,
			IntegrityCheckReport.Error.NO_ERROR, "unregisteringPluginsStart", null)); //$NON-NLS-1$
		...
		registrationReport.add(new ReportItemImpl(IntegrityCheckReport.Severity.INFO, null,
			IntegrityCheckReport.Error.NO_ERROR, "unregisteringPluginsFinish", //$NON-NLS-1$
			Integer.valueOf(registeredPlugins.size())));
		registrationReport.add(new ReportItemImpl(IntegrityCheckReport.Severity.INFO, null,
			IntegrityCheckReport.Error.NO_ERROR, "unregisteringFragmentsFinish", //$NON-NLS-1$
			Integer.valueOf(registeredFragments.size())));
	...
	private void unregisterPlugin(final PluginDescriptorImpl descr)
	{
		...
		registrationReport.add(new ReportItemImpl(IntegrityCheckReport.Severity.INFO, null,
			IntegrityCheckReport.Error.NO_ERROR, "pluginUnregistered", descr.getUniqueId())); //$NON-NLS-1$
	}

	private void unregisterFragment(final PluginFragmentImpl fragment)
	{
		PluginDescriptorImpl descr = (PluginDescriptorImpl) registeredPlugins.get(fragment
			.getPluginId());
		if( descr != null )
		{
		...
		registrationReport.add(new ReportItemImpl(IntegrityCheckReport.Severity.INFO, null,
			IntegrityCheckReport.Error.NO_ERROR, "fragmentUnregistered", fragment.getUniqueId())); //$NON-NLS-1$
	}
	...
	/**
	 * @see org.java.plugin.registry.PluginRegistry#isExtensionPointAvailable(java.lang.String,
	 *      java.lang.String)
	 */
	...
	public Collection<PluginDescriptor> getPluginDescriptors()
	{
		...
		return registeredPlugins.isEmpty() ? empty_collection : Collections
			.unmodifiableCollection(registeredPlugins.values());
	}
	...
	public Collection<PluginFragment> getPluginFragments()
	{
		...
		return registeredFragments.isEmpty() ? empty_collection : Collections
			.unmodifiableCollection(registeredFragments.values());
	}
	...
	/**
	 * @see org.java.plugin.registry.PluginRegistry#checkIntegrity(org.java.plugin.PathResolver)
	 */
		...
		final boolean includeRegistrationReport)
	{
		...
		IntegrityChecker intergityCheckReport = new IntegrityChecker(this,
			includeRegistrationReport ? registrationReport : empty_collection);
	...
	/**
	 * @see org.java.plugin.registry.PluginRegistry#makeUniqueId(java.lang.String,
	 *      java.lang.String)
	 */
	...
	/**
	 * @see org.java.plugin.registry.PluginRegistry#registerListener(org.java.plugin.registry.PluginRegistry.RegistryChangeListener)
	 */
	...
	/**
	 * @see org.java.plugin.registry.PluginRegistry#unregisterListener(org.java.plugin.registry.PluginRegistry.RegistryChangeListener)
	 */
	...
	void fireEvent(final RegistryChangeDataImpl data)
	{
		...
		// make local copy
		RegistryChangeListener[] arr = listeners.toArray(new RegistryChangeListener[listeners
			.size()]);
	...
	private static final class RegistryChangeDataImpl implements RegistryChangeData
	{
		...
		public Set<String> addedExtensions(final String extensionPointUid)
		{
			...
			for( Iterator<Entry<String, String>> it = addedExtensions.entrySet().iterator(); it
				.hasNext(); )
			{
				entry = it.next();
		...
		public Set<String> removedExtensions(final String extensionPointUid)
		{
			final Set<String> result = new HashSet<String>();
			Entry<String, String> entry;
			for( Iterator<Entry<String, String>> it = removedExtensions.entrySet().iterator(); it
				.hasNext(); )
			{
				entry = it.next();
				if( entry.getValue().equals(extensionPointUid) )
		...
		public Set<String> modifiedExtensions(final String extensionPointUid)
		{
			final Set<String> result = new HashSet<String>();
			Entry<String, String> entry;
			for( Iterator<Entry<String, String>> it = modifiedExtensions.entrySet().iterator(); it
				.hasNext(); )
			{
				entry = it.next();
				if( entry.getValue().equals(extensionPointUid) )
