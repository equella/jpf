...
public class StandardPluginLocation implements PluginLocation
{
	...
	public static PluginLocation create(final File file) throws MalformedURLException
	{
		if( file.isDirectory() )
		{
			...
			return (manifestUrl == null) ? null : new StandardPluginLocation(IoUtil.file2url(file),
				manifestUrl);
		}
		...
		URL manifestUrl = getManifestUrl(file);
		return (manifestUrl == null) ? null : new StandardPluginLocation(new URL("jar:" //$NON-NLS-1$
			+ IoUtil.file2url(file).toExternalForm() + "!/"), manifestUrl); //$NON-NLS-1$
	}
	...
	private static URL getManifestUrl(final File file) throws MalformedURLException
	{
		...
		url = new URL("jar:" //$NON-NLS-1$
			+ IoUtil.file2url(file).toExternalForm() + "!/plugin-fragment.xml"); //$NON-NLS-1$
		if( IoUtil.isResourceExists(url) )
		{
			return url;
		}
		url = new URL("jar:" //$NON-NLS-1$
			+ IoUtil.file2url(file).toExternalForm() + "!/META-INF/plugin.xml"); //$NON-NLS-1$
		if( IoUtil.isResourceExists(url) )
		{
			return url;
		}
		url = new URL("jar:" //$NON-NLS-1$
			+ IoUtil.file2url(file).toExternalForm() + "!/META-INF/plugin-fragment.xml"); //$NON-NLS-1$
		if( IoUtil.isResourceExists(url) )
		{
			return url;
		}
